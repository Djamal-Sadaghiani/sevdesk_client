...
module OpenapiClient
  class ExportInvoiceSevQueryParameterFilter
    # Type of invoices you want to export 1. RE - Rechnung 2. SR - Stornorechnung 3. TR - Teilrechnung 4. AR - Abschlagsrechnung 5. ER - Endrechnung 6. WKR - Wiederkehrende Rechnung 7. MA - Mahnung
    ...
    def self.openapi_types
      {
        :'invoice_type' => :'Array<String>',
        :'start_date' => :'Time',
        :'end_date' => :'Time',
        :'contact' => :'ExportInvoiceSevQueryParameterFilterContact',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ExportInvoiceSevQueryParameterFilter` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ExportInvoiceSevQueryParameterFilter`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'invoice_type')
        if (value = attributes[:'invoice_type']).is_a?(Array)
    ...
    def hash
      [invoice_type, start_date, end_date, contact, start_amount, end_amount].hash
    end
