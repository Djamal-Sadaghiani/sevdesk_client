...
module OpenapiClient
  class SendInvoiceViaEMailRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SendInvoiceViaEMailRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SendInvoiceViaEMailRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'send_xml')
        self.send_xml = attributes[:'send_xml']
      end
    end
    ...
    def hash
      [to_email, subject, text, copy, additional_attachments, cc_email, bcc_email, send_xml].hash
    end
