...
module OpenapiClient
  class CreateCreditNoteFromInvoice201ResponseObjects
    attr_accessor :credit_note

    # An array of creditNote positions
    attr_accessor :credit_note_pos
    ...
    def self.openapi_types
      {
        :'credit_note' => :'ModelCreditNoteResponse',
        :'credit_note_pos' => :'Array<ModelCreditNotePosResponse>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CreateCreditNoteFromInvoice201ResponseObjects` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CreateCreditNoteFromInvoice201ResponseObjects`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'credit_note_pos')
        if (value = attributes[:'credit_note_pos']).is_a?(Array)
          self.credit_note_pos = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          credit_note_pos == o.credit_note_pos &&
          discount == o.discount
    end
    ...
    def hash
      [credit_note, credit_note_pos, discount].hash
    end
