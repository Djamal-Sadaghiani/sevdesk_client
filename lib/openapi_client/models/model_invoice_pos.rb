  ...
  class ModelInvoicePos
    ...
    def self.openapi_types
      {
        ...
        :'create' => :'Time',
        :'update' => :'Time',
        :'invoice' => :'ModelInvoicePosInvoice',
        :'part' => :'ModelCreditNotePosPart',
        :'quantity' => :'Float',
        :'price' => :'Float',
        :'name' => :'String',
        :'unity' => :'ModelInvoicePosUnity',
        :'sev_client' => :'ModelInvoicePosSevClient',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ModelInvoicePos` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ModelInvoicePos`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, object_name, map_all, create, update, invoice, part, quantity, price, name, unity, sev_client, position_number, text, discount, tax_rate, sum_discount, sum_net_accounting, sum_tax_accounting, sum_gross_accounting, price_net, price_gross, price_tax].hash
    end
