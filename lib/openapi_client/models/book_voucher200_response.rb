  ...
  class BookVoucher200Response
    ...
    def self.openapi_types
      {
        ...
        :'create' => :'Time',
        :'credit_note' => :'BookVoucher200ResponseCreditNote',
        :'from_status' => :'String',
        :'to_status' => :'String',
        :'ammount_payed' => :'String',
        :'booking_date' => :'Time',
        :'sev_client' => :'BookInvoice200ResponseSevClient'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::BookVoucher200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::BookVoucher200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
