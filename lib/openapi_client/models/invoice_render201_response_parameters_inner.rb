...
module OpenapiClient
  class InvoiceRender201ResponseParametersInner
    attr_accessor :key
    ...
    def self.openapi_types
      {
        :'key' => :'String',
        :'name' => :'String',
        :'values' => :'Array<InvoiceRender201ResponseParametersInnerValuesInner>',
        :'visible' => :'Boolean',
        :'value' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InvoiceRender201ResponseParametersInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InvoiceRender201ResponseParametersInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [key, name, values, visible, value].hash
    end
