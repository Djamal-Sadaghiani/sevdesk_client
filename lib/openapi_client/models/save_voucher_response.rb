  ...
  class SaveVoucherResponse
    ...
    def self.openapi_types
      {
        ...
        :'voucher_pos' => :'Array<ModelVoucherPosResponse>',
        :'filename' => :'File'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SaveVoucherResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SaveVoucherResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'voucher_pos')
        if (value = attributes[:'voucher_pos']).is_a?(Array)
          self.voucher_pos = value
        end
      end
    ...
    def hash
      [voucher, voucher_pos, filename].hash
    end
