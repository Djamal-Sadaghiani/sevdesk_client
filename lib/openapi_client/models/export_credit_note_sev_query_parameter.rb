...
module OpenapiClient
  class ExportCreditNoteSevQueryParameter
    # Limit export
    attr_accessor :limit
    ...
    def self.openapi_types
      {
        ...
        :'model_name' => :'Object',
        :'object_name' => :'Object',
        :'filter' => :'ExportCreditNoteSevQueryParameterFilter'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ExportCreditNoteSevQueryParameter` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ExportCreditNoteSevQueryParameter`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
