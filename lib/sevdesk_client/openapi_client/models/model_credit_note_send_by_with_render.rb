...
module OpenapiClient
  class ModelCreditNoteSendByWithRender
    ...
    def self.openapi_types
      {
        ...
        :'pages' => :'Integer',
        :'doc_id' => :'String',
        :'parameters' => :'Array<InvoiceRender201ResponseParametersInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ModelCreditNoteSendByWithRender` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ModelCreditNoteSendByWithRender`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'thumbs')
        if (value = attributes[:'thumbs']).is_a?(Array)
          self.thumbs = value
        end
      end
    ...
    def hash
      [thumbs, pages, doc_id, parameters].hash
    end
