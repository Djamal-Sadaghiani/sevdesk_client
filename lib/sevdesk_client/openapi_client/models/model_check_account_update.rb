  ...
  class ModelCheckAccountUpdate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ModelCheckAccountUpdate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ModelCheckAccountUpdate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'default_account')
        ...
        self.default_account = DEFAULT_ACCOUNT::N0
      end

      if attributes.key?(:'status')
        self.status = attributes[:'status']
      else
        self.status = STATUS::N100
      end
    ...
    def valid?
      ...
      default_account_validator = EnumAttributeValidator.new('Integer', [0, 1])
      return false unless default_account_validator.valid?(@default_account)
      status_validator = EnumAttributeValidator.new('Integer', [0, 100])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Integer', [0, 100])
    ...
    def hash
      [name, type, import_type, currency, default_account, status, auto_map_transactions, accounting_number].hash
    end
